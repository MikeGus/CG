...
void MainWindow::add_figure()
{
    if (scene->data.buffer.size() > 2) {
        ...
        case SPLIT:
            ...
            break;
        case SPLIT_HOLE:
            scene->data.split.holes.push_back(scene->data.buffer);
            break;
        case FIGURE:
            ...
            break;
        case FIGURE_HOLE:
            scene->data.fig.holes.push_back(scene->data.buffer);
            break;
        }
...
void MainWindow::on_radioSplitHoles_toggled(bool checked)
{
    add_figure();
    if (checked) {
        scene->data.type_of_input = SPLIT_HOLE;
    }
}
...
void MainWindow::on_radioFigureHoles_toggled(bool checked)
{
    add_figure();
    if (checked) {
        scene->data.type_of_input = FIGURE_HOLE;
    }
}
